...
public abstract class WorldGenWizardryStructure implements IWorldGenerator {
	...
	public void generate(Random random, int chunkX, int chunkZ, Level world, IChunkGenerator chunkGenerator, IChunkProvider chunkProvider){
		...
		if(canGenerate(random, world, chunkX, chunkZ)){
			...
			// DEBUG
//			level.setBlockAndUpdate(new BlockPos(box.minX, box.minY, box.minZ), Blocks.CONCRETE.defaultBlockState().withProperty(BlockColored.COLOR, EnumDyeColor.MAGENTA));
//			level.setBlockAndUpdate(new BlockPos(box.maxX, box.maxY, box.maxZ), Blocks.CONCRETE.defaultBlockState().withProperty(BlockColored.COLOR, EnumDyeColor.MAGENTA));
