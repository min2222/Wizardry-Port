...
public class PotionContainment extends PotionMagicEffect {
	...
	public PotionContainment(MobEffectCategory category, int liquidColour){
		super(category, liquidColour, new ResourceLocation(Wizardry.MODID, "textures/gui/potion_icons/containment.png"));
	}
	...
	public void applyEffectTick(LivingEntity target, int strength){
		...
		Vec3 origin = GeometryUtils.getCentre(NbtUtils.readBlockPos(target.getPersistentData().getCompound(ENTITY_TAG)));
		...
		{
			target.push(0.15 * Math.signum(x - target.getX()), 0.15 * Math.signum(y - target.getY()), 0.15 * Math.signum(z - target.getZ()));
			EntityUtils.undoGravity(target);
			if(target.level.isClientSide){
				target.level.playLocalSound(target.getX(), target.getY(), target.getZ(), WizardrySounds.ENTITY_FORCEFIELD_DEFLECT,
		...
		if(target.getEffect(this).getDuration() <= 1) target.getPersistentData().remove(ENTITY_TAG);
	...
	public static void onLivingUpdateEvent(LivingEvent.LivingTickEvent event){
		...
		if(event.getEntity().tickCount % 20 == 0 && event.getEntity().getPersistentData().contains(ENTITY_TAG)
				&& !event.getEntity().hasEffect(WizardryPotions.CONTAINMENT.get())){
